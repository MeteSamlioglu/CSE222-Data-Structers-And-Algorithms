...
/*                      Post operations/add post/interactions...               */
	...
	public void unFollow(Account Acc)
	{
		...
		if(this.isAccountFollowed(Acc.getID( )) == true)
		{
			for(int i = 0 ; i < 100; i++)
			{
				if(Following[i].getID( ) == Acc.getID( ))
				{
					Following[i] = null;
					index = i;
					break;
			...
			for(int i = index; i < 99; i++) /* Slide all the elements one left */ 
			...
			for(int i = 1 ; i < 100; i++)
			{	
				if(Acc.Followers[i].getID( ) == this.getID( )) 
				{
					Acc.Followers[i] = null;
					index = i;
					break;
				}
			}
			for(int i = index; i < 99; i++) 
				Acc.Followers[i] = Acc.Followers[i + 1];

			Acc.Followers[99] = null;
	...
	public boolean isBlocked(int AccID)
	{
		...
		for(int i = 0; i < 100; i++)
		{
			if( (this.Blocks[i]!=null ) && (AccID == this.Blocks[i].getID( )))
			{
				isAccBlocked = true;
				break;
			}
